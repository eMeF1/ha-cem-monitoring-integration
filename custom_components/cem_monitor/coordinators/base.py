...
def _create_session(hass: HomeAssistant, verify_ssl: bool = True) -> ClientSession:
    ...
    """
    if verify_ssl:
        return cast(ClientSession, async_get_clientsession(hass))
...
class CEMBaseCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    ...
    def _get_auth_credentials(self) -> tuple[str | None, str | None]:
        """
    ...
    async def _ensure_token(self, context: str = "") -> tuple[str, str | None]:
        """
    ...
    async def _handle_401_error(
        self,
        err: Exception,
        api_call: Callable[[str, str | None], Awaitable[Any]],
        context: str,
        error_prefix: str,
    ) -> Any:
        """
        ...
        try:
            ...
            if is_401_error(retry_err):
                raise UpdateFailed(
                    f"{error_prefix} failed: authentication failed after token refresh"
                ) from retry_err
            raise UpdateFailed(
                f"{error_prefix} failed after token refresh: {retry_err}"
            ) from retry_err
...
class CEMAuthCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    ...
    async def _async_update_data(self) -> dict[str, Any]:
        ...
        return {
            "connected": True,
            "token_expires_at_iso": self.token_expires_at.isoformat()
